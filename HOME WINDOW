...
private final JButton btnSearch, btnAdd, btnViewProfile, btnDelete,
btnRefresh;
...
private final String[] column = {"SL NO", "ID", "Name", "Gender", "Age",
"Weight", "Blood Group", "Last Donation", "Mobile No", "Home Division",
"Home District"};
...
frameHome.setIconImage(icon.getImage());
//setting window icon
...
//setting panel size
topPanel.setBackground(clrRed); //background
color: Red
JPanel panelLeft = new JPanel();
JPanel panelRight = new JPanel();
//left panle work
panelLeft.setLayout(new BorderLayout());
panelLeft.setBackground(clrRed); //background
color: Red
poster = new ImageIcon(getClass().getResource("images/poster.png"));
//create imageIcon for poster
JLabel lblPoster = new JLabel();
lblPoster.setIcon(poster); //set image for
poster
panelLeft.add(lblPoster, BorderLayout.PAGE_START);
//left panel work done
//right panel work
panelRight.setBorder(new EmptyBorder(50, 50, 50, 50));
//setting panel border
panelRight.setPreferredSize(new Dimension(350, 180));
//setting panel size
panelRight.setBackground(clrRed); //background
color: Red
panelRight.setLayout(new BorderLayout());
JLabel lblDay = new JLabel();
lblDay.setFont(new Font("Sans", Font.BOLD, 20)); //font:
Sans, Style: Bold, Size: 20
lblDay.setForeground(Color.WHITE); //font color:
White
lblDay.setHorizontalAlignment(JLabel.LEFT); //left
alignment
JLabel lblTime = new JLabel();
lblTime.setFont(new Font("Sans", Font.BOLD, 30)); //font:
Sans, Style: Bold, Size: 30
lblTime.setForeground(Color.WHITE); //font color:
White
lblTime.setHorizontalAlignment(JLabel.CENTER); //center
alignment
JLabel lblDate = new JLabel();
lblDate.setFont(new Font("Sans", Font.BOLD, 18)); //font:
Sans, Style: Bold, Size: 18
lblDate.setForeground(Color.WHITE); //font color:
White
lblDate.setHorizontalAlignment(JLabel.RIGHT); //right
alignment
SimpleDateFormat ftDay = new SimpleDateFormat("EEEE");
//setting format for day
SimpleDateFormat ftTime = new SimpleDateFormat("hh:mm:ss a");
//setting format for time
SimpleDateFormat ftDate = new SimpleDateFormat("dd MMMM yyyy");
...
Timer time = new Timer(100, updateClockAction); //set timer to
...
topPanel.add(panelLeft, BorderLayout.WEST); //add
panelLeft to topPanel
topPanel.add(panelRight, BorderLayout.EAST); //add
panelRight to topPanel
//topPanel work done
...
midPanel.setBackground(clrLightRed); //background
color: Light Red
...
panelUpper.setPreferredSize(new Dimension(1000, 45));
//setting panel size
panelUpper.setBackground(clrLightRed);
//background color: light red
JLabel lblSearch = new JLabel("Search By :");
lblSearch.setFont(new Font("Sans", Font.BOLD, 14)); //font:
Sans, Style: Bold, Size: 20
lblSearch.setForeground(Color.BLACK); //font
color: black
lblSearch.setBounds(20, 15, 100, 20); //setting
location and size for search label
String search[] = {"Blood Group", "Name", "Home Division", "Home
District"};
JComboBox cbSearch = new JComboBox(search);
//combobox for search options
cbSearch.setBounds(120, 13, 150, 25); //setting
location and size for search combobox
JTextField tfSearch = new JTextField();
tfSearch.setFont(new Font("Sans", Font.PLAIN, 14)); //setting
font for search TextField
tfSearch.setBounds(280, 13, 200, 25); //setting
location and size of TextField
btnSearch = new JButton("Search");
btnSearch.setBounds(490, 13, 100, 25); //setting
location and size of search button
btnSearch.setBackground(Color.DARK_GRAY);
//button background color: Dark Gray
btnSearch.setForeground(Color.WHITE); //button
text color: Whit
...
panelLower.setBackground(clrLightRed);
//background color: Light Red
...
tblData.setBackground(clrYellow); //table
background color: Yellow
tblData.setRowHeight(30); //set row height
tblData.setFont(new Font("Sans", Font.PLAIN, 14)); //font:
Sans Style: Plain Size: 14
tblData.setDefaultEditor(Object.class, null); //set cell noneditable
...
JScrollPane sp = new JScrollPane(tblData); //added
table into a scroll pane
...
//anchoring buttons into lower right side
panelBtn.setBackground(clrLightRed); //background
color: Light Red
panelBtn.setBorder(new EmptyBorder(0, 0, 5, 30)); //setting
panel border
btnAdd = new JButton("Add New Donor");
btnAdd.setBackground(clrGreen); //button
background color: Green
btnAdd.setForeground(Color.WHITE); //button text
color: White
...
btnViewProfile.setBackground(clrBlue); //button
background color: Blue
btnViewProfile.setForeground(Color.white); //button
text color: White
btnViewProfile.setCursor(new Cursor(Cursor.HAND_CURSOR));
//set button cursor
btnViewProfile.setVisible(false); //set button
invisible
btnViewProfile.addActionListener(new ActionListener() {
@Override
public void actionPerformed(ActionEvent ae) {
int row = tblData.getSelectedRow(); //getting view
index of selected row
row = tblData.convertRowIndexToModel(row); //getting
model index of selected row
int id = Integer.parseInt(tblData.getModel().getValueAt(row,
1).toString());
System.out.println("ID: " + id + " Selected");
new ViewProfile(id);
}});
btnDelete = new JButton("Delete");
btnDelete.setBackground(clrRed); //button
background color: Red
btnDelete.setForeground(Color.white); //button text
color: White
...
btnRefresh.setBackground(clrYellow); //button
background color: Red
btnRefresh.setForeground(Color.BLACK); //button
text color: White
...
tblData.getSelectionModel().addListSelectionListener(new
...
//setting border for bottom panel
bottomPanel.setPreferredSize(new Dimension(1000, 50));
//setting panel size
bottomPanel.setBackground(clrRed); //background
color: Red
JLabel lblFooter = new JLabel();
String footer = "“Share A Little, Care A Little – Donate Blood.”";
lblFooter.setText(footer); //set text of footnote
lblFooter.setFont(new Font("Serif", Font.ITALIC, 18)); //font:
Serif, Style: Italic, Size: 18
lblFooter.setForeground(Color.WHITE); //cont
color: White
bottomPanel.add(lblFooter); //add footnote to
panel
...
JOptionPane.showMessageDialog(frameHome, "No Data Found!",
