...
frameDevelopers.setIconImage(icon.getImage()); //setting
window icon
//setting up the frame
frameDevelopers.setSize(600, 400);
frameDevelopers.setLayout(new BorderLayout()); //frame
layout: Border Layout
frameDevelopers.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
//setting default close operation
frameDevelopers.setLocationRelativeTo(null); //to center
the window
frameDevelopers.setResizable(false); //disable
window resize
//setting up the panel
panelMain = new JPanel();
panelMain.setBackground(clrYellow); //background
color: Yellow
...
TitledBorder ttlDevInfo = BorderFactory.createTitledBorder(blueline);
//create titled border
ttlDevInfo.setTitle(" Developers "); //set title
ttlDevInfo.setTitleColor(clrBlue); //title color: Blue
panelMain.setBorder(new CompoundBorder(margin, ttlDevInfo));
...
taCntct1.setText("Mobile: +919361045302\n"
+ "Gmail: eswar.g2019@vitstudent.ac.in");
...
taCntct2.setText("Mobile: +91 9361045302\n"
+ "Gmail: developer2@gmail.com");
panelMain.add(taCntct2);
btnOk = new JButton("OK");
btnOk.setBackground(clrBlue); //button
background color: Red
btnOk.setForeground(Color.white); //button text
color: White
